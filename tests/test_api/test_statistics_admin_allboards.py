"""Тесты для API статистики администратора (все доски)."""
import allure
import pytest
...
def test_get_all_boards_admin_valid(auth_service, statistics_service):
    ...
    if isinstance(body, dict):
        assert "boards" in body and isinstance(body["boards"], list), \
            "Ожидаемый 'boards' в тексте ответа"
        assert "total" in body and isinstance(body["total"], int), \
            "Ожидаемый 'total' в тексте ответа"
...
def test_invalid_integer_params_trigger_validation(auth_service,statistics_service,
                                                   skip, limit, archived):
    token = auth_service.get_token_for_me("admin@example.com", "admin123")
    response = statistics_service.get_all_boards_admin(
        {"skip": skip, "limit": limit, "archived": archived}, token)
